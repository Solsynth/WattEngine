...
Ideask links one GitHub repository to one board (`WtBroad`). Linked issues become tasks, and supported task changes synchronize back to GitHub. This is a GitHub App integration: users install the app and select repositories, but never create webhooks or provide GitHub tokens to Ideask.
...
1. Request an installation URL and open the returned URL in a browser or popup.
...
4. List the repositories available to that installation and let the user select exactly one.
...
6. Show current state with `GET /api/github/broads/{broadId}`. Offer manual repair through `POST /api/github/broads/{broadId}/sync`, and unlink with `DELETE /api/github/broads/{broadId}`.
...
3. Return to WattEngine, choose one authorized repository, and confirm linking.
4. Existing GitHub issues import as tasks. New tasks create GitHub issues. Changes to title, content, tags, and open/closed state synchronize in both directions.
